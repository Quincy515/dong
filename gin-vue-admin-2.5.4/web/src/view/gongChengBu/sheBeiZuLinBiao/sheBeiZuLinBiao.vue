  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="租赁ID">
            
             <el-input v-model.number="searchInfo.zuLinId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="租赁ID" prop="zuLinId" width="120" />
        <el-table-column align="left" label="租赁用途" prop="zuLinYongTu" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateSheBeiZuLinBiaoFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="租赁ID:"  prop="zuLinId" >
          <el-input v-model.number="formData.zuLinId" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="租赁用途:"  prop="zuLinYongTu" >
          <el-input v-model="formData.zuLinYongTu" :clearable="true"  placeholder="请输入" />
        </el-form-item>
      </el-form>
      <template #footer>
...
<script setup>
import {
  createSheBeiZuLinBiao,
  deleteSheBeiZuLinBiao,
  deleteSheBeiZuLinBiaoByIds,
  updateSheBeiZuLinBiao,
  findSheBeiZuLinBiao,
  getSheBeiZuLinBiaoList
} from '@/api/sheBeiZuLinBiao'
...
const getTableData = async() => {
  const table = await getSheBeiZuLinBiaoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteSheBeiZuLinBiaoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSheBeiZuLinBiaoFunc = async(row) => {
    const res = await findSheBeiZuLinBiao({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.resheBeiZuLinBiao
...
// 删除行
const deleteSheBeiZuLinBiaoFunc = async (row) => {
    const res = await deleteSheBeiZuLinBiao({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createSheBeiZuLinBiao(formData.value)
                  break
                case 'update':
                  res = await updateSheBeiZuLinBiao(formData.value)
                  break
                default:
                  res = await createSheBeiZuLinBiao(formData.value)
                  break
