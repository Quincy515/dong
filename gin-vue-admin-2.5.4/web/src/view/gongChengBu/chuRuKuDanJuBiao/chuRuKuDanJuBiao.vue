<template>
  <div>
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="采购计划ID">
           ...
           <el-form-item label="类型 1. 入库 2. 出库" prop="chuRuKuDanJuLeiXing">
            <el-select v-model="searchInfo.chuRuKuDanJuLeiXing" clearable placeholder="请选择" @clear="()=>{searchInfo.chuRuKuDanJuLeiXing=undefined}">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="类型 1. 入库 2. 出库" prop="chuRuKuDanJuLeiXing" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.chuRuKuDanJuLeiXing,chuRuKuDanJuLeiXingOptions) }}
            </template>
        </el-table-column>
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateChuRuKuDanJuBiaoFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="类型 1. 入库 2. 出库:"  prop="chuRuKuDanJuLeiXing" >
          <el-select v-model="formData.chuRuKuDanJuLeiXing" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in chuRuKuDanJuLeiXingOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
...
import {
  ...
  deleteChuRuKuDanJuBiao,
  deleteChuRuKuDanJuBiaoByIds,
  updateChuRuKuDanJuBiao,
  findChuRuKuDanJuBiao,
  getChuRuKuDanJuBiaoList
...
const getTableData = async() => {
  const table = await getChuRuKuDanJuBiaoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteChuRuKuDanJuBiaoFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteChuRuKuDanJuBiaoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateChuRuKuDanJuBiaoFunc = async(row) => {
    const res = await findChuRuKuDanJuBiao({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteChuRuKuDanJuBiaoFunc = async (row) => {
    const res = await deleteChuRuKuDanJuBiao({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
