<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="接收方 id">
             ...
             <el-input v-model.number="searchInfo.receiverId" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="发送方 id">
            
             <el-input v-model.number="searchInfo.senderId" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="状态 0.未读 1.已读">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="接收方 id" prop="receiverId" width="120" />
        <el-table-column align="left" label="跳转地址" prop="redirectUrl" width="120" />
        <el-table-column align="left" label="发送方 id" prop="senderId" width="120" />
        <el-table-column align="left" label="状态 0.未读 1.已读" prop="status" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateXiaoXiTongZhiFunc(scope.row)">变更</el-button>
...
import {
  createXiaoXiTongZhi,
  deleteXiaoXiTongZhi,
  deleteXiaoXiTongZhiByIds,
  updateXiaoXiTongZhi,
  findXiaoXiTongZhi,
  getXiaoXiTongZhiList
...
const getTableData = async() => {
  const table = await getXiaoXiTongZhiList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteXiaoXiTongZhiByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateXiaoXiTongZhiFunc = async(row) => {
    const res = await findXiaoXiTongZhi({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteXiaoXiTongZhiFunc = async (row) => {
    const res = await deleteXiaoXiTongZhi({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
