<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="出勤" prop="chuQin" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.chuQin) }}</template>
        </el-table-column>
        <el-table-column align="left" label="所属" prop="group" width="120" />
         <el-table-column align="left" label="日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.riQi) }}</template>
         </el-table-column>
        <el-table-column align="left" label="详情" prop="xiangQing" width="120" />
        <el-table-column align="left" label="姓名" prop="xingMing" width="120" />
        <el-table-column align="left" label="休假请假出差" prop="xiuJiaQingJiaChuCha" width="120">
            <template #default="scope">{{ formatBoolean(scope.row.xiuJiaQingJiaChuCha) }}</template>
        </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateMeiRiKaoQinFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="日期:"  prop="riQi" >
          <el-date-picker v-model="formData.riQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
...
const onSubmit = () => {
  page.value = 1
  pageSize.value = 10
  if (searchInfo.value.chuQin === ""){
      searchInfo.value.chuQin=null
  }
  if (searchInfo.value.xiuJiaQingJiaChuCha === ""){
      searchInfo.value.xiuJiaQingJiaChuCha=null
  }
  getTableData()
}
...
const getTableData = async() => {
  const table = await getMeiRiKaoQinList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteMeiRiKaoQinByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateMeiRiKaoQinFunc = async(row) => {
    const res = await findMeiRiKaoQin({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteMeiRiKaoQinFunc = async (row) => {
    const res = await deleteMeiRiKaoQin({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
