<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="报销人员">
             ...
             <el-input v-model.number="searchInfo.baoXiaoRenYuan" placeholder="搜索条件" />
        ...
        <el-form-item label="首次报销ID">
            
             <el-input v-model.number="searchInfo.shouCiBaoXiaoId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="报销类型" prop="baoXiaoLeiXing" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.baoXiaoLeiXing,baoXiaoLeiXingOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="报销名称" prop="baoXiaoMingCheng" width="120" />
        ...
        <el-table-column align="left" label="回单地址-可多图片上传NULL" prop="huiDanDiZhi" width="120" />
        ...
        <el-table-column align="left" label="首次报销ID" prop="shouCiBaoXiaoId" width="120" />
        ...
        <el-table-column align="left" label="状态" prop="zhuangTai" width="120" />
         <el-table-column align="left" label="最晚付款日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.zuiWanFuKuanRiQi) }}</template>
         </el-table-column>
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateWaiBuFeiYongQingKuanBiaoFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="最晚付款日期:"  prop="zuiWanFuKuanRiQi" >
          <el-date-picker v-model="formData.zuiWanFuKuanRiQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
      </el-form>
      <template #footer>
...
import {
  ...
  deleteWaiBuFeiYongQingKuanBiao,
  deleteWaiBuFeiYongQingKuanBiaoByIds,
  updateWaiBuFeiYongQingKuanBiao,
  findWaiBuFeiYongQingKuanBiao,
  getWaiBuFeiYongQingKuanBiaoList
...
const getTableData = async() => {
  const table = await getWaiBuFeiYongQingKuanBiaoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteWaiBuFeiYongQingKuanBiaoFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteWaiBuFeiYongQingKuanBiaoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateWaiBuFeiYongQingKuanBiaoFunc = async(row) => {
    const res = await findWaiBuFeiYongQingKuanBiao({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteWaiBuFeiYongQingKuanBiaoFunc = async (row) => {
    const res = await deleteWaiBuFeiYongQingKuanBiao({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
