  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="分包商ID">
            
             <el-input v-model.number="searchInfo.fenBaoShangId" placeholder="搜索条件" />
        ...
        <el-form-item label="委托人">
         <el-input v-model="searchInfo.weiTuoRen" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="分包商ID" prop="fenBaoShangId" width="120" />
        ...
        <el-table-column align="left" label="合同名称" prop="heTongMingCheng" width="120" />
         <el-table-column align="left" label="进场日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.jinChangRiQi) }}</template>
         </el-table-column>
         <el-table-column align="left" label="退场日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.tuiChangRiQi) }}</template>
         </el-table-column>
        <el-table-column align="left" label="委托人" prop="weiTuoRen" width="120" />
        <el-table-column align="left" label="委托人手机" prop="weiTuoRenShouJi" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateFenBaoShangYuXiangMuGuanLianBiaoFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="进场日期:"  prop="jinChangRiQi" >
          <el-date-picker v-model="formData.jinChangRiQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
        <el-form-item label="退场日期:"  prop="tuiChangRiQi" >
          <el-date-picker v-model="formData.tuiChangRiQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
...
import {
  ...
  deleteFenBaoShangYuXiangMuGuanLianBiao,
  deleteFenBaoShangYuXiangMuGuanLianBiaoByIds,
  updateFenBaoShangYuXiangMuGuanLianBiao,
  findFenBaoShangYuXiangMuGuanLianBiao,
  getFenBaoShangYuXiangMuGuanLianBiaoList
...
const getTableData = async() => {
  const table = await getFenBaoShangYuXiangMuGuanLianBiaoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteFenBaoShangYuXiangMuGuanLianBiaoFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteFenBaoShangYuXiangMuGuanLianBiaoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateFenBaoShangYuXiangMuGuanLianBiaoFunc = async(row) => {
    const res = await findFenBaoShangYuXiangMuGuanLianBiao({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteFenBaoShangYuXiangMuGuanLianBiaoFunc = async (row) => {
    const res = await deleteFenBaoShangYuXiangMuGuanLianBiao({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
