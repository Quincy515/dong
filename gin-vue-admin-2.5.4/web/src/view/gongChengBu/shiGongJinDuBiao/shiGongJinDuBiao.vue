  ...
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="工作名称ID">
            
             <el-input v-model.number="searchInfo.gongZuoMingChengId" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="工作名称ID" prop="gongZuoMingChengId" width="120" />
        <el-table-column align="left" label="所属" prop="group" width="120" />
         <el-table-column align="left" label="实际开始日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.shiJiKaiShiRiQi) }}</template>
         </el-table-column>
        <el-table-column align="left" label="实际天数" prop="shiJiTianShu" width="120" />
         <el-table-column align="left" label="实际完成日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.shiJiWanChengRiQi) }}</template>
         </el-table-column>
        ...
        <el-table-column align="left" label="完成百分比" prop="wanChengBaiFenBi" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            ...
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateShiGongJinDuBiaoFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="实际开始日期:"  prop="shiJiKaiShiRiQi" >
          <el-date-picker v-model="formData.shiJiKaiShiRiQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
        ...
        <el-form-item label="实际完成日期:"  prop="shiJiWanChengRiQi" >
          <el-date-picker v-model="formData.shiJiWanChengRiQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
        ...
        <el-form-item label="完成百分比:"  prop="wanChengBaiFenBi" >
          <el-input-number v-model="formData.wanChengBaiFenBi"  style="width:100%" :precision="2" :clearable="true"  />
        </el-form-item>
...
import {
  ...
  deleteShiGongJinDuBiao,
  deleteShiGongJinDuBiaoByIds,
  updateShiGongJinDuBiao,
  findShiGongJinDuBiao,
  getShiGongJinDuBiaoList
...
const getTableData = async() => {
  const table = await getShiGongJinDuBiaoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteShiGongJinDuBiaoFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteShiGongJinDuBiaoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateShiGongJinDuBiaoFunc = async(row) => {
    const res = await findShiGongJinDuBiao({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteShiGongJinDuBiaoFunc = async (row) => {
    const res = await deleteShiGongJinDuBiao({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
