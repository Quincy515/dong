<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="供应商名称">
             ...
             <el-input v-model.number="searchInfo.gongYingShangMingCheng" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="发票抬头" prop="faPiaoTaiTou" width="120" />
        <el-table-column align="left" label="法人身份证" prop="faRenShenFenZheng" width="120" />
        <el-table-column align="left" label="公司地址" prop="gongSiDiZhi" width="120" />
        <el-table-column align="left" label="供应商名称" prop="gongYingShangMingCheng" width="120" />
        <el-table-column align="left" label="所属" prop="group" width="120" />
        <el-table-column align="left" label="开户行" prop="kaiHuHang" width="120" />
        <el-table-column align="left" label="联系人" prop="lianXiRen" width="120" />
        <el-table-column align="left" label="联系人手机" prop="lianXiRenShouJi" width="120" />
        ...
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateGongYingShangBiaoFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteGongYingShangBiao,
  deleteGongYingShangBiaoByIds,
  updateGongYingShangBiao,
  findGongYingShangBiao,
  getGongYingShangBiaoList
...
const getTableData = async() => {
  const table = await getGongYingShangBiaoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteGongYingShangBiaoFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteGongYingShangBiaoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateGongYingShangBiaoFunc = async(row) => {
    const res = await findGongYingShangBiao({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteGongYingShangBiaoFunc = async (row) => {
    const res = await deleteGongYingShangBiao({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
