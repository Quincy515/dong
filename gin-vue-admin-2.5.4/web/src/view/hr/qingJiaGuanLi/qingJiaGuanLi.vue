    ...
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束时间"></el-date-picker>
        ...
        <el-form-item label="进度">
         <el-input v-model="searchInfo.jinDu" placeholder="搜索条件" />

        </el-form-item>
           <el-form-item label="请假类型" prop="leiXing">
            <el-select v-model="searchInfo.leiXing" clearable placeholder="请选择" @clear="()=>{searchInfo.leiXing=undefined}">
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="所属" prop="group" width="120" />
         <el-table-column align="left" label="结束日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.jieShuRiQi) }}</template>
         </el-table-column>
        <el-table-column align="left" label="进度" prop="jinDu" width="120" />
         <el-table-column align="left" label="开始日期" width="180">
            <template #default="scope">{{ formatDate(scope.row.kaiShiRiQi) }}</template>
         </el-table-column>
        <el-table-column align="left" label="请假类型" prop="leiXing" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.leiXing,qing_jia_lei_xingOptions) }}
            </template>
        </el-table-column>
        <el-table-column align="left" label="审核人" prop="shenHeRen" width="120" />
         <el-table-column align="left" label="审核时间" width="180">
            <template #default="scope">{{ formatDate(scope.row.shenHeShiJian) }}</template>
         </el-table-column>
        <el-table-column align="left" label="审核详情" prop="shenHeXiangQing" width="120" />
        <el-table-column align="left" label="审核状态" prop="shenHeZhuangTai" width="120" />
        <el-table-column align="left" label="事由" prop="shiYou" width="120" />
        <el-table-column align="left" label="天数" prop="tianShu" width="120" />
        <el-table-column align="left" label="姓名" prop="xingMing" width="120" />
        <el-table-column align="left" label="按钮组">
            <template #default="scope">
            <el-button type="primary" link icon="edit" size="small" class="table-button" @click="updateQingJiaGuanLiFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
        ...
        <el-form-item label="结束日期:"  prop="jieShuRiQi" >
          <el-date-picker v-model="formData.jieShuRiQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
        ...
        <el-form-item label="开始日期:"  prop="kaiShiRiQi" >
          <el-date-picker v-model="formData.kaiShiRiQi" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
        <el-form-item label="请假类型:"  prop="leiXing" >
          <el-select v-model="formData.leiXing" placeholder="请选择" style="width:100%" :clearable="true" >
            <el-option v-for="(item,key) in qing_jia_lei_xingOptions" :key="key" :label="item.label" :value="item.value" />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="审核时间:"  prop="shenHeShiJian" >
          <el-date-picker v-model="formData.shenHeShiJian" type="date" style="width:100%" placeholder="选择日期" :clearable="true"  />
        </el-form-item>
...
import {
  createQingJiaGuanLi,
  deleteQingJiaGuanLi,
  deleteQingJiaGuanLiByIds,
  updateQingJiaGuanLi,
  findQingJiaGuanLi,
  getQingJiaGuanLiList
...
const getTableData = async() => {
  const table = await getQingJiaGuanLiList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteQingJiaGuanLiByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateQingJiaGuanLiFunc = async(row) => {
    const res = await findQingJiaGuanLi({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteQingJiaGuanLiFunc = async (row) => {
    const res = await deleteQingJiaGuanLi({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
